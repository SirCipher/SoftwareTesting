...
public class BookGenerator_en extends BookGenerator {
    ...
    boolean isValidLanguage( String word, String wikiText ) {
        ...
        if( props != null ) {
            ...
            if(correctWord != null && BookUtils.calcDiff( word, correctWord ) <= 3){
                misSpellingWeight += 512;
            }
        }
        props = BookUtils.parseRule( wikiText, "label|en", 0 );
        if( props != null ) {
            String labelType = props.getProperty( "1" );
            if( "obsolete".equals( labelType ) ) {
                misSpellingWeight += 512;
            }
            String subType = props.getProperty( "2" );
            if( "slang".equals( subType ) ) {
                misSpellingWeight += 512;
            }
        }
        if( misSpellingWeight > wikiText.length() ) {
            return false;
        }
